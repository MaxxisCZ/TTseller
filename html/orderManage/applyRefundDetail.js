define(function(require, exports, module) {
    ...
    var applyRefundDetail = new Vue({
        el: '#applyRefundDetail',
        template: _g.getTemplate('orderManage/applyRefundDetail-main-V'),
        data: {
            isShow: 1,
            code: '1234',
            inputCode: '',
        ...
        methods: {
            onUserDetailTap: function() {
                _g.openWin({
                    ...
                    pageParam: {
                        userid: applyRefundDetail.detail.userID,
                    }
                });
            },
            ...
            picker: function() {
                api.openPicker({
                    ...
                    if (ret) {
                        ...
                        applyRefundDetail.detail.getTime = ret.year+'-'+ret.month+'-'+ret.day+ ' '+ret.hour+':'+ret.minute;
                    } else {
            ...
            onSubmitTap: function() {
                ...
                Http.ajax({
                    data: {
                        ...
                        orderid: orderid,
                        orderstatus: applyRefundDetail.detail.selected,
                        addrid: applyRefundDetail.detail.addrID,
                        jhtime: applyRefundDetail.detail.getTime,
                        shrmobile: applyRefundDetail.detail.recPhone,
                    },
            ...
            onRefundSingleTap: function(index) {
                api.confirm({
                    ...
                    if(ret.buttonIndex == 2) {
                        if(applyRefundDetail.setPwd == true) {
                            _g.openWin({
                                header: {
                                    data: {
                                        title: '支付密码',
                            ...
                            Http.ajax({
                                data: {
                                    ...
                                    token: _g.getLS('UserInfo').token,
                                    orderid: orderid,
                                    ordetailid: applyRefundDetail.detail.goods[ret.value.index].goodOrderID
                                },
            ...
            onAmountTap: function() {
                ...
                var standard = [];
                var sum = [];
                for (var i = 0; i < applyRefundDetail.detail.goods.length; i++) {
                    goodsID.push(applyRefundDetail.detail.goods[i].goodsID)
                    standard.push(applyRefundDetail.detail.goods[i].standardID);
                    sum.push(applyRefundDetail.detail.goods[i].amount);
                }
                applyRefundDetail.goodsIDArr = goodsID.join(',');
                applyRefundDetail.standardArr = standard.join(',');
                applyRefundDetail.sumArr = sum.join(',');
                Http.ajax({
                    data: {
                        loginname: _g.getLS('UserInfo').loginname,
                        agentid: _g.getLS('UserInfo').agentid,
                        ...
                        token: _g.getLS('UserInfo').token,
                        orderid: orderid,
                        pid: applyRefundDetail.goodsIDArr,
                        ggid: applyRefundDetail.standardArr,
                        ptcount: applyRefundDetail.sumArr,
                    },
    ...
    var getOrderData = function() {
        Http.ajax({
            ...
            success: function(ret) {
                if (ret.zt == 1) {
                    var data = ret.data;
                    applyRefundDetail.detail = getOderDetail(data);
                    makeCodeTap();
                } else if(ret.zt == -1) {
                    _g.rmLS('UserInfo');
                    api && api.openWin({
        ...
        Http.ajax({
            ...
            success: function(ret) {
                if (ret.zt == 1) {
                    ...
                    applyRefundDetail.deliveryTime = data;
                } else if(ret.zt == -1) {
                    _g.rmLS('UserInfo');
                    api && api.openWin({
                        name: 'account-login-win',
    ...
    function makeCodeTap() {
        applyRefundDetail.code = Vcode.getVerificationCode();
    }
    function havePsw() {
        Http.ajax({
            data: {
            ...
            success: function(ret) {
                if(ret.zt == 1) {
                    if(ret.data.payflag == 1) {
                        applyRefundDetail.setPwd = true;
                    } else {
                        applyRefundDetail.setPwd = false;
                    }
                } else if(ret.zt == -1) {
                    _g.hideProgress();
                    _g.rmLS('UserInfo');
                ...
                if(ret.buttonIndex == 2) {
                    Http.ajax({
                        data: {
                            ...
                            staffid: UserInfo.staffid,
                            token: UserInfo.token,
                            data: printOrder(applyRefundDetail.detail),
                        },
                        isSync: true,
                ...
                if(ret.buttonIndex == 2) {
                    Http.ajax({
                        data: {
                            ...
                            staffid: UserInfo.staffid,
                            token: UserInfo.token,
                            data: printProduct(applyRefundDetail.detail.goods),
                        },
                        isSync: true,
    ...
    api.addEventListener({
        ...
        setTimeout(function() {
            applyRefundDetail.reason = ret.value.reason;
        }, 500);
        if(applyRefundDetail.setPwd == true) {
            _g.openWin({
                header: {
    ...
    api.addEventListener({
        ...
        applyRefundDetail.detail.addr = ret.value.changeAddress;
        applyRefundDetail.detail.addrID = ret.value.addrID;
    });
