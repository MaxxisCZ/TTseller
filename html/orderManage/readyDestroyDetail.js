define(function(require, exports, module) {
    ...
    var readyDestroyDetail = new Vue({
        el: '#readyDestroyDetail',
        template: _g.getTemplate('orderManage/readyDestroyDetail-main-V'),
        data: {
            isShow: 1,
            code: '1234',
            inputCode: '',
        ...
        methods: {
            onUserDetailTap: function() {
                _g.openWin({
                    ...
                    pageParam: {
                        userid: readyDestroyDetail.detail.userID,
                    }
                });
            },
            ...
            picker: function() {
                api.openPicker({
                    ...
                    if (ret) {
                        ...
                        readyDestroyDetail.detail.getTime = ret.year+'-'+ret.month+'-'+ret.day+ ' '+ret.hour+':'+ret.minute;
                    } else {
            ...
            onSubmitTap: function() {
                ...
                Http.ajax({
                    data: {
                        ...
                        orderid: orderid,
                        orderstatus: readyDestroyDetail.detail.selected,
                        addrid: readyDestroyDetail.detail.addrID,
                        jhtime: readyDestroyDetail.detail.getTime,
                        shrmobile: readyDestroyDetail.detail.recPhone,
                    },
            ...
            onDistoryTap: function() {
                api.confirm({
                    ...
                    if(ret.buttonIndex == 2) {
                        if(readyDestroyDetail.setPwd == true) {
                            _g.openWin({
                                header: {
                                    data: {
                                        title: '支付密码',
            ...
            onAmountTap: function() {
                ...
                var standard = [];
                var sum = [];
                for (var i = 0; i < readyDestroyDetail.detail.goods.length; i++) {
                    goodsID.push(readyDestroyDetail.detail.goods[i].goodsID)
                    standard.push(readyDestroyDetail.detail.goods[i].standardID);
                    sum.push(readyDestroyDetail.detail.goods[i].amount);
                }
                readyDestroyDetail.goodsIDArr = goodsID.join(',');
                readyDestroyDetail.standardArr = standard.join(',');
                readyDestroyDetail.sumArr = sum.join(',');
                Http.ajax({
                    data: {
                        loginname: _g.getLS('UserInfo').loginname,
                        agentid: _g.getLS('UserInfo').agentid,
                        ...
                        token: _g.getLS('UserInfo').token,
                        orderid: orderid,
                        pid: readyDestroyDetail.goodsIDArr,
                        ggid: readyDestroyDetail.standardArr,
                        ptcount: readyDestroyDetail.sumArr,
                    },
    ...
    var getOrderData = function() {
        Http.ajax({
            ...
            success: function(ret) {
                if (ret.zt == 1) {
                    var data = ret.data;
                    readyDestroyDetail.detail = getOderDetail(data);
                    makeCodeTap();
                } else if(ret.zt == -1) {
                    _g.rmLS('UserInfo');
                    api && api.openWin({
        ...
        Http.ajax({
            ...
            success: function(ret) {
                if (ret.zt == 1) {
                    ...
                    readyDestroyDetail.deliveryTime = data;
                } else if(ret.zt == -1) {
                    _g.rmLS('UserInfo');
                    api && api.openWin({
                        name: 'account-login-win',
    ...
    function makeCodeTap() {
        readyDestroyDetail.code = Vcode.getVerificationCode();
    }
    function havePsw() {
        Http.ajax({
            data: {
            ...
            success: function(ret) {
                if(ret.zt == 1) {
                    if(ret.data.payflag == 1) {
                        readyDestroyDetail.setPwd = true;
                    } else {
                        readyDestroyDetail.setPwd = false;
                    }
                } else if(ret.zt == -1) {
                    _g.rmLS('UserInfo');
                    api && api.openWin({
    ...
    api.addEventListener({
        ...
        if(ret.value.index == 0) {
            ...
            //                 staffid: UserInfo.staffid,
            //                 token: UserInfo.token,
            //                 data: printOrder(readyDestroyDetail.detail),
            //             },
            //             isSync: true,
            ...
            //                 staffid: UserInfo.staffid,
            //                 token: UserInfo.token,
            //                 data: printProduct(readyDestroyDetail.detail.goods),
            //             },
            //             isSync: true,
    ...
    api.addEventListener({
        ...
        Http.ajax({
            data: {
                ...
                token: _g.getLS('UserInfo').token,
                orderid: orderid,
                ordetailid: readyDestroyDetail.detail.goods[ret.value.index].goodOrderID
            },
    ...
    api.addEventListener({
        name: 'orderManage-readyDestroyDetail-received',
    }, function(ret, err) {
        Http.ajax({
            data: {
                loginname: _g.getLS('UserInfo').loginname,
    ...
    api.addEventListener({
        ...
        readyDestroyDetail.detail.addr = ret.value.changeAddress;
        readyDestroyDetail.detail.addrID = ret.value.addrID;
    });
