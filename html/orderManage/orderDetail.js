define(function(require, exports, module) {
    ...
    var orderDetail = new Vue({
        el: '#orderDetail',
        template: _g.getTemplate('orderManage/orderDetail-main-V'),
        data: {
            isShow: 1,
            code: '1234',
            inputCode: '',
        ...
        methods: {
            onUserDetailTap: function() {
                _g.openWin({
                    ...
                    pageParam: {
                        userid: orderDetail.detail.userID,
                    }
                });
            },
            ...
            picker: function() {
                api.openPicker({
                    ...
                    if (ret) {
                        ...
                        orderDetail.detail.getTime = ret.year+'-'+ret.month+'-'+ret.day+ ' '+ret.hour+':'+ret.minute;
                    } else {
            ...
            onSubmitTap: function() {
                ...
                Http.ajax({
                    data: {
                        ...
                        orderid: orderid,
                        orderstatus: orderDetail.detail.selected,
                        addrid: orderDetail.detail.addrID,
                        jhtime: orderDetail.detail.getTime,
                        shrmobile: orderDetail.detail.recPhone,
                    },
                        ...
                        if(orderDetail.setPwd == true) {
                            ...
                            Http.ajax({
                                data: {
                                    ...
                                    token: _g.getLS('UserInfo').token,
                                    orderid: orderid,
                                    ordetailid: orderDetail.detail.goods[index].goodOrderID
                                },
            ...
            onAmountTap: function() {
                if(orderDetail.setPwd == true) {
                    ...
                    var standard = [];
                    var sum = [];
                    for (var i = 0; i < orderDetail.detail.goods.length; i++) {
                        goodsID.push(orderDetail.detail.goods[i].goodsID)
                        standard.push(orderDetail.detail.goods[i].standardID);
                        sum.push(orderDetail.detail.goods[i].amount);
                    }
                    orderDetail.goodsIDArr = goodsID.join(',');
                    orderDetail.standardArr = standard.join(',');
                    orderDetail.sumArr = sum.join(',');
                    Http.ajax({
                        data: {
                            loginname: _g.getLS('UserInfo').loginname,
                            agentid: _g.getLS('UserInfo').agentid,
                            ...
                            token: _g.getLS('UserInfo').token,
                            orderid: orderid,
                            pid: orderDetail.goodsIDArr,
                            ggid: orderDetail.standardArr,
                            ptcount: orderDetail.sumArr,
                        },
    ...
    var getOrderData = function() {
        Http.ajax({
            ...
            success: function(ret) {
                if (ret.zt == 1) {
                    var data = ret.data;
                    orderDetail.detail = getOderDetail(data);
                    makeCodeTap();
                } else if(ret.zt == -1) {
                    _g.hideProgress();
                    _g.rmLS('UserInfo');
    ...
    function makeCodeTap() {
        orderDetail.code = Vcode.getVerificationCode();
    }
    function havePsw() {
        Http.ajax({
            data: {
            ...
            success: function(ret) {
                if(ret.zt == 1) {
                    if(ret.data.payflag == 1) {
                        orderDetail.setPwd = true;
                    } else {
                        orderDetail.setPwd = false;
                    }
                } else if(ret.zt == -1) {
                    _g.rmLS('UserInfo');
                    api && api.openWin({
                ...
                if(ret.buttonIndex == 2) {
                    Http.ajax({
                        data: {
                            ...
                            staffid: UserInfo.staffid,
                            token: UserInfo.token,
                            data: printOrder(orderDetail.detail),
                        },
                        isSync: true,
                ...
                if(ret.buttonIndex == 2) {
                    Http.ajax({
                        data: {
                            ...
                            staffid: UserInfo.staffid,
                            token: UserInfo.token,
                            data: printProduct(orderDetail.detail.goods),
                        },
                        isSync: true,
    ...
    api.addEventListener({
        ...
        Http.ajax({
            data: {
                ...
                token: _g.getLS('UserInfo').token,
                orderid: orderid,
                ordetailid: orderDetail.detail.goods[ret.value.index].goodOrderID
            },
            url: '/jiekou/agentht/order/drawback.aspx',
            success: function(ret) {
                if(ret.zt == 1) {
    ...
    api.addEventListener({
        ...
        setTimeout(function() {
            orderDetail.reason = ret.value.reason;
        }, 500);
        if(orderDetail.setPwd == true) {
            _g.openWin({
                header: {
    ...
    api.addEventListener({
        ...
        orderDetail.detail.addr = ret.value.changeAddress;
        orderDetail.detail.addrID = ret.value.addrID;
    });
