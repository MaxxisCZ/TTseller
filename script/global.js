...
window.VERSION = '1.2.1'; // 代码版本号, 每次发布之前, 请更新, 小版本号自增+1
...
APPMODE == 'dev' && (function() {
  //  CONFIG.HOST = 'http://139.196.27.119:89';  //开发包外网域名
    // CONFIG.HOST = 'http://114.219.172.117:2554';  //开发包域名
     CONFIG.HOST = 'https://tt.iptt.cn';  //正式包域名
    var isApp = !!window.localStorage.getItem('isApp');
    if (window.APPMODE == 'dev' && !isApp) {
        // 如果是开发模式并且不是app启动
        CONFIG.HOST = 'https://tt.iptt.cn';
    }
})();
APPMODE == 'pub' && (function() {
    // CONFIG.HOST = 'http://192.168.1.199:2554';  //内网域名
    //连接本地数据库CONFIG.HOST = 'http://192.168.3.110';
    CONFIG.HOST = 'https://tt.iptt.cn';  //开发包外网域名
  //  CONFIG.HOST = 'http://192.168.3.110';  //外网域名
})();
...
Vue.filter('translate-price',function(price){
...
Vue.filter('trans-photo', function(photo) {
    return photo ? (photo) : CONFIG.DEFAULT_PHOTO;
});
...
apiready = function() {
    ...
    seajs.use([
        $('#entry').data('path'),
        'MOD/toast'
    ], function(Entry, Toast, Common, Vue, VueTouch, Mock) {
        window.Toast = Toast;
        _g.fixStatusBar();
        console.log('entry')
    });
